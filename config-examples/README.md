...
The provided [sample `docker-compose.yml`](docker-compose.yml.sample) file can be used as-is. If you use your machine for multiple purposes, we recommend putting your AIS containers in a separate stack. This is so it's easier to maintain -- strictly speaking, you *can* put all containers in a single stack.

Specifically:
```bash
sudo mkdir /opt/ais && sudo chmod a+rwx /opt/ais && cd /opt/ais
curl https
```

## ShipXplorer Configuration

- the (ShipXplorer)[https://github.com/sdr-enthusiasts/docker-shipxplorer] container contains the basics needed to get started:
  - AIS-Catcher, which decodes AIS from a SDR or other external data source. It also provides a web page.
  - `sxfeeder`, which feeds this data to ShipXplorer in a controlled manner
