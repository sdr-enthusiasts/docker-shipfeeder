...
SX_RTLSDR_GAIN=auto
SX_UDP_FEEDS=ip1:port1,ip2:port2
...
| `SX_UDP_FEEDS`            | Optional. Defines target UDP feeds in addition to ShipExplorer. Format: `UDP_FEEDS=domain1.com:port1,domain2,com:port2,...` |
...
To feed `aprs.fi` using HTTP, you firwst need to create an AIS feeder account at that service. Note that creating accounts is limited to licensed amateurn radio operators. With this account, you can create a personalized feeder URL, for example `http://aprs.fi/jsonais/post/aBcDeFgHiJkL`.
